...
class OssClient
{
	...
	public function generateMultiuploadParts($file_size, $partSize = 5242880)
	{
		...
		while ($size_count > 0) {
			$size_count -= $partSize;
			$values[] = array(self::OSS_SEEK_TO => ($partSize * $i), self::OSS_LENGTH => (($size_count > 0) ? $partSize : ($size_count + $partSize)),);
			$i++;
		}
		return $values;
	}
	...
	private function stringToSignSorted($string_to_sign)
	{
		...
		if ($index === 1) return $string_to_sign;
	...
	static $OSS_ACL_TYPES = array(self::OSS_ACL_TYPE_PRIVATE, self::OSS_ACL_TYPE_PUBLIC_READ, self::OSS_ACL_TYPE_PUBLIC_READ_WRITE);
