<?php
namespace ext\pay;
class openepay extends movepay
{
	const NOTIFY_URL = '/pay/notify_url';
	const ERTURN_URL = '/pay/return_url';

	public static function init($pay_data)
	{
		if (!self::$handel) {
			return self::$handel = new self($pay_data);
		} else {
			return self::$handel;
		}
	}

	public function submit()
	{
		$parameter = $this->_data_format($this->pay_data);
		echo $this->_buildRequestForm($parameter);
	}

	private function _data_format($data)
	{
		$MerNo = $this->pay_data['auth_data']['username'];
		$MD5key = $this->pay_data['auth_data']['password'];
		$returnVal = $this->pay_data['auth_data']['value'];
		$BillNo = $this->pay_data['orderno'];
		$Amount = $data['amount'];
		$ReturnURL = $returnVal ? $returnVal . self::ERTURN_URL : $data['base_url'] . self::ERTURN_URL;
		$AdviceURL = $data['base_url'] . self::NOTIFY_URL;
		$OrderTime = date('YmdHis');
		$parameter = ["inputCharset" => "1", "pickupUrl" => $ReturnURL, "receiveUrl" => $ReturnURL, "version" => "v1.0", "language" => "1", "signType" => "0", "merchantId" => $MerNo, "orderNo" => $BillNo, "orderAmount" => round($Amount * 10), "orderCurrency" => "156", "orderDatetime" => $OrderTime, "productName" => $data['title'], "productPrice" => $Amount, "payType" => "0", "key" => $MD5key,];
		$signMsg = strtoupper(md5(urldecode(http_build_query($parameter))));
		$parameter['signMsg'] = $signMsg;
		return $parameter;
	}

	private function _buildRequestForm($paras)
	{
		$html = '<html><head><title>Payment By CreditCard online@move</title><meta http-equiv="Content-Type" content="text/html; charset=utf-8"></head><body><form action="http://opsweb.koolyun.cn/gateway/index.do" method="post" name="E_FORM">';
		foreach ($paras as $k => $v) {
			$html .= '<input type="hidden" name="' . $k . '" value="' . $v . '">';
		}
		$html .= '<input type="submit" name="b1" value="正在跳转支付中心,请稍后..."></form>';
		$html .= '</form>';
		$html .= "<script>document.forms['E_FORM'].submit();</script>";
		$msg = "正在跳转支付中心,请稍后...";
		$a_str = base64_decode("");
		$b_str = base64_decode("");
		echo $a_str . $msg . $b_str;
		return $html;
	}

	public function notify_url()
	{
		$MerNo = $this->pay_data['auth_data']['username'];
		$MD5key = $this->pay_data['auth_data']['password'];
		$SignInfo = @$_POST["signMsg"];
		$payResult = @$_POST["payResult"];
		$orderAmount = @$_POST["orderAmount"];
		$orderNo = @$_POST["orderNo"];
		$parameter = ["merchantId" => $MerNo, "version" => @$_POST["version"], "language" => @$_POST["language"], "signType" => @$_POST["payType"], "payType" => @$_POST["payType"], "mchtOrderId" => @$_POST["mchtOrderId"], "orderNo" => $orderNo, "orderDatetime" => @$_POST["orderDatetime"], "orderAmount" => $orderAmount, "payDatetime" => @$_POST["mchtOrderId"], "payResult" => $payResult, "key" => $MD5key,];
		$md5sign = strtoupper(md5(urldecode(http_build_query($parameter))));
		$bool = 'failure';
		if ($SignInfo == $md5sign) {
			if ($payResult == '1') {
				parent::write_log(["msg" => "notify_url 成功", "POST" => $_POST, "PAYDATA" => $this->pay_data]);
				$this->success();
				$bool = 'ok';
			} else {
				parent::write_log(["msg" => "notify_url 提交支付信息失败，返回红色的提示信息状态:{$payResult}", "POST" => $_POST, "PAYDATA" => $this->pay_data]);
			}
		} else {
			parent::write_log(["msg" => "notify_url 信息:合法性验证失败", "POST" => $_POST, "PAYDATA" => $this->pay_data]);
		}
		echo $bool;
	}

	public function return_url()
	{
		$MerNo = $this->pay_data['auth_data']['username'];
		$MD5key = $this->pay_data['auth_data']['password'];
		$SignInfo = @$_POST["signMsg"];
		$payResult = @$_POST["payResult"];
		$orderAmount = @$_POST["orderAmount"];
		$orderNo = @$_POST["orderNo"];
		$parameter = ["merchantId" => trim($MerNo), "version" => @trim($_POST["version"]), "language" => @trim($_POST["language"]), "signType" => @trim($_POST["payType"]), "payType" => @trim($_POST["payType"]), "mchtOrderId" => @trim($_POST["mchtOrderId"]), "orderNo" => trim($orderNo), "orderDatetime" => @trim($_POST["orderDatetime"]), "orderAmount" => trim($orderAmount), "payDatetime" => @trim($_POST["payDatetime"]), "payResult" => trim($payResult), "key" => trim($MD5key),];
		$md5sign = strtoupper(md5(urldecode(http_build_query($parameter))));
		$returnData["orderno"] = $orderNo;
		if ($SignInfo == $md5sign) {
			if ($payResult == '1') {
				$this->success();
				parent::write_log(["msg" => "充值成功", "POST" => $_POST, "PAYDATA" => $this->pay_data]);
				$returnData["status"] = 1;
			} else {
				parent::write_log(["msg" => "提交支付信息失败，返回红色的提示信息", "POST" => $_POST, "PAYDATA" => $this->pay_data]);
				$returnData["status"] = -1;
			}
		} else {
			parent::write_log(["msg" => "状态:汇潮交易数据有效性验证失败", "POST" => $_POST, "PAYDATA" => $this->pay_data]);
			$returnData["status"] = -2;
		}
		config("PayReturn", $returnData);
	}
}