...
class gopay extends movepay
{
	...
	private function _data_format($data)
	{
		...
		if (defined("MS_GOPAY") && MS_GOPAY) {
			$MerNoArr = explode("|", $MerNo);
			$MerNo = $MerNoArr['0'];
			$VerficationCode = $MD5key;
			$virCardNoIn = $MerNoArr['1'];
		} else {
			$MerNo = "0000001502";
		...
		$parameter = ["version" => "2.1", "charset" => "1", "language" => "1", "signType" => "1", "tranCode" => "8888", "merchantID" => $MerNo, "merOrderNum" => $BillNo, "tranAmt" => $Amount, "feeAmt" => 0, "currencyType" => "156", "frontMerUrl" => $ReturnURL, "backgroundMerUrl" => $AdviceURL, "tranDateTime" => $OrderTime, "virCardNoIn" => $virCardNoIn, "tranIP" => isset($_SERVER['REMOTE_ADDR']) && $_SERVER['REMOTE_ADDR'] ? $_SERVER['REMOTE_ADDR'] : "127.0.0.1", "isRepeatSubmit" => "0", "goodsName" => $Remark, "gopayServerTime" => $gopayServerTime];
	...
	{
		if (defined("MS_GOPAY") && MS_GOPAY) {
			$action = 'https://gateway.gopay.com.cn/Trans/WebClientAction.do';
		} else {
			$action = 'https://gatewaymer.gopay.com.cn/Trans/WebClientAction.do';
		}
	...
	public function notify_url()
	{
		...
		$signValue = $_POST["signValue"];
		if (defined("MS_GOPAY") && MS_GOPAY) {
			$VerficationCode = $this->pay_data['auth_data']['password'];
		} else {
			$VerficationCode = "11111aaaaa";
		}
