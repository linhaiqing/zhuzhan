...
if(w.addEventListener){w.addEventListener("resize",callMedia,false);}else if(w.attachEvent){w.attachEvent("onresize",callMedia);}})(this);;(function(){"use strict";function setup($){$.fn._fadeIn=$.fn.fadeIn;var noOp=$.noop||function(){};var msie=/MSIE/.test(navigator.userAgent);var ie6=/MSIE 6.0/.test(navigator.userAgent)&&!/MSIE 8.0/.test(navigator.userAgent);var mode=document.documentMode||0;var setExpr=$.isFunction(document.createElement('div').style.setExpression);$.blockUI=function(opts){install(window,opts);};$.unblockUI=function(opts){remove(window,opts);};$.growlUI=function(title,message,timeout,onClose){var $m=$('<div class="growlUI"></div>');if(title)$m.append('<h1>'+title+'</h1>');if(message)$m.append('<h2>'+message+'</h2>');if(timeout===undefined)timeout=3000;var callBlock=function(opts){opts=opts||{};$.blockUI({message:$m,fadeIn:typeof opts.fadeIn!=='undefined'?opts.fadeIn:700,fadeOut:typeof opts.fadeOut!=='undefined'?opts.fadeOut:1000,timeout:typeof opts.timeout!=='undefined'?opts.timeout:timeout,centerY:false,showOverlay:false,onUnblock:onClose,css:$.blockUI.defaults.growlCSS});};callBlock();var nonmousedOpacity=$m.css('opacity');$m.mouseover(function(){callBlock({fadeIn:0,timeout:30000});var displayBlock=$('.blockMsg');displayBlock.stop();displayBlock.fadeTo(300,1);}).mouseout(function(){$('.blockMsg').fadeOut(1000);});};$.fn.block=function(opts){if(this[0]===window){$.blockUI(opts);return this;}
var fullOpts=$.extend({},$.blockUI.defaults,opts||{});this.each(function(){var $el=$(this);if(fullOpts.ignoreIfBlocked&&$el.data('blockUI.isBlocked'))
return;$el.unblock({fadeOut:0});});return this.each(function(){if($.css(this,'position')=='static'){this.style.position='relative';$(this).data('blockUI.static',true);}
this.style.zoom=1;install(this,opts);});};$.fn.unblock=function(opts){if(this[0]===window){$.unblockUI(opts);return this;}
return this.each(function(){remove(this,opts);});};$.blockUI.version=2.66;$.blockUI.defaults={message:'<div>Loading data...</div>',title:null,draggable:true,theme:false,css:{padding:'20px',margin:0,width:'30%',top:'40%',left:'35%',textAlign:'center',color:'#fff',border:'none',backgroundColor:'#000',cursor:'wait',opacity:.7,'border-radius':'4px'},themedCSS:{width:'30%',top:'40%',left:'35%'},overlayCSS:{backgroundColor:'#000',opacity:0.3,cursor:'wait'},cursorReset:'default',growlCSS:{width:'350px',top:'10px',left:'',right:'10px',border:'none',padding:'5px',opacity:0.6,cursor:'default',color:'#fff',backgroundColor:'#000','-webkit-border-radius':'10px','-moz-border-radius':'10px','border-radius':'10px'},iframeSrc:/^https/i.test(window.location.href||'')?'javascript:false':'about:blank',forceIframe:false,baseZ:1000,centerX:true,centerY:true,allowBodyStretch:true,bindEvents:true,constrainTabKey:true,fadeIn:200,fadeOut:400,timeout:0,showOverlay:true,focusInput:true,focusableElements:':input:enabled:visible',onBlock:null,onUnblock:null,onOverlayClick:null,quirksmodeOffsetHack:4,blockMsgClass:'blockMsg',ignoreIfBlocked:false};var pageBlock=null;var pageBlockEls=[];function install(el,opts){var css,themedCSS;var full=(el==window);var loading=(opts&&opts.message!==undefined?opts.message:undefined);var msg='<h4><i class="fa fa-spinner fa-spin"></i>  '+loading+'</h4>';opts=$.extend({},$.blockUI.defaults,opts||{});if(opts.ignoreIfBlocked&&$(el).data('blockUI.isBlocked'))
...
if(typeof define==='function'&&define.amd&&define.amd.jQuery){define(['jquery'],setup);}else{setup(jQuery);}})();function QR8bitByte(data){this.mode=QRMode.MODE_8BIT_BYTE;this.data=data;}
...
return $table;}
return this.each(function(){var element=options.render=="canvas"?createCanvas():createTable();$(element).appendTo(this);});};})(jQuery);(function($){var $ie6=(function(){return false===$.support.boxModel&&$.support.objectAll&&$.support.leadingWhitespace;})();$.jGrowl=function(m,o){if($('#jGrowl').length===0)
...
if($.isFunction($(this).data('jGrowl.instance')[m])){$(this).data('jGrowl.instance')[m].apply($(this).data('jGrowl.instance'),$.makeArray(args).slice(1));}else{$(this).data('jGrowl.instance').create(m,o);}});}};$.extend($.fn.jGrowl.prototype,{defaults:{pool:6,header:'',group:'',sticky:false,position:'bottom-left',glue:'after',theme:'default',themeState:'highlight',corners:'',check:50,life:4000,closeDuration:'fast',openDuration:'fast',easing:'swing',closer:true,closeTemplate:'<i class="fa fa-times" />',closerTemplate:'<div>[ close all ]</div>',log:function(){},beforeOpen:function(){},afterOpen:function(){},open:function(){},beforeClose:function(){},close:function(){},animateOpen:{opacity:'show'},animateClose:{opacity:'hide'}},notifications:[],element:null,interval:null,create:function(message,options){var o=$.extend({},this.defaults,options);if(typeof o.speed!=='undefined'){o.openDuration=o.speed;o.closeDuration=o.speed;}
...
$(this).trigger('jGrowl.close');}).bind('jGrowl.close',function(){$(this).animate(o.animateClose,o.closeDuration,o.easing,function(){if($.isFunction(o.close)){if(o.close.apply(notification,[notification,message,o,self.element])!==false)
$(this).remove();}else{$(this).remove();}});}).trigger('jGrowl.beforeOpen');if(o.corners!==''&&$.fn.corner!==undefined)$(notification).corner(o.corners);if($('div.jGrowl-notification:parent',self.element).length>1&&$('div.jGrowl-closer',self.element).length===0&&this.defaults.closer!==false){$(this.defaults.closerTemplate).addClass('jGrowl-closer '+this.defaults.themeState+' ui-corner-all').addClass(this.defaults.theme).appendTo(self.element).animate(this.defaults.animateOpen,this.defaults.speed,this.defaults.easing).bind("click.jGrowl",function(){$(this).siblings().trigger("jGrowl.beforeClose");if($.isFunction(self.defaults.closer)){self.defaults.closer.apply($(this).parent()[0],[$(this).parent()[0]]);}});}},update:function(){if(this.notifications.length>0)
this.render(this.notifications.shift());var jGrowl_notifications=$(this.element).find('div.jGrowl-notification:parent');var count=jGrowl_notifications.length;jGrowl_notifications.each(function(){if($(this).data("jGrowl")!==undefined&&$(this).data("jGrowl").created!==undefined){if(count>5){$(this).remove();count--;}
else if($(this).data("jGrowl").created.getTime()+parseInt($(this).data("jGrowl").life,10)<(new Date()).getTime()){$(this).trigger('jGrowl.close');}}});if($(this.element).find('div.jGrowl-notification:parent').length<2){$(this.element).find('div.jGrowl-closer').animate(this.defaults.animateClose,this.defaults.speed,this.defaults.easing,function(){$(this).remove();});}},startup:function(e){this.element=$(e).addClass('jGrowl').append('<div class="jGrowl-notification"></div>');this.interval=setInterval(function(){$(e).data('jGrowl.instance').update();},parseInt(this.defaults.check,10));if($ie6){$(this.element).addClass('ie6');}},shutdown:function(){$(this.element).removeClass('jGrowl').find('div.jGrowl-notification').trigger('jGrowl.close').parent().empty();clearInterval(this.interval);},close:function(){$(this.element).find('div.jGrowl-notification').each(function(){$(this).trigger('jGrowl.beforeClose');});}});$.jGrowl.defaults=$.fn.jGrowl.prototype.defaults;})(jQuery);(function(){var initializing=false;window.JQClass=function(){};JQClass.classes={};JQClass.extend=function extender(prop){var base=this.prototype;initializing=true;var prototype=new this();initializing=false;for(var name in prop){prototype[name]=typeof prop[name]=='function'&&typeof base[name]=='function'?(function(name,fn){return function(){var __super=this._super;this._super=function(args){return base[name].apply(this,args||[]);};var ret=fn.apply(this,arguments);this._super=__super;return ret;};})(name,prop[name]):prop[name];}
...
this._registerSubscribedHubs();return proxy;};hubConnection.fn.init.prototype=hubConnection.fn;$.hubConnection=hubConnection;}(window.jQuery,window));(function($,undefined){$.signalR.version="2.2.2";}(window.jQuery));(function($){var data_click="unobtrusiveAjaxClick",data_target="unobtrusiveAjaxClickTarget",data_validation="unobtrusiveValidation";function getFunction(code,argNames){var fn=window,parts=(code||"").split(".");while(fn&&parts.length){fn=fn[parts.shift()];}
...
asyncRequest(this,{url:this.action,type:this.method||"GET",data:clickInfo.concat($(this).serializeArray())});});}(jQuery));window.emotify=(function(){var emotify,EMOTICON_RE,emoticons={},lookup=[];emotify=function(txt,callback){callback=callback||function(img,title,smiley,text){return'<img title="'+title+'" style="max-height:80px;" src="'+img+'" class="smiley"/>';};return txt.replace(EMOTICON_RE,function(a,b,text){var i=0,smiley=text,e=emoticons[text];if(!e){while(i<lookup.length&&!lookup[i].regexp.test(text)){i++};var item=lookup[i];if(item){smiley=item.name;e=emoticons[smiley];}}
return e?(b+callback(e[0],e[1],smiley,text)):a;});};emotifyTable=function(txt,callback){callback=callback||function(img,title,smiley,text){return'<img title="'+title+'" height="20px" src="'+img+'" class="smiley"/>';};return txt.replace(EMOTICON_RE,function(a,b,text){var i=0,smiley=text,e=emoticons[text];if(!e){while(i<lookup.length&&!lookup[i].regexp.test(text)){i++};smiley=lookup[i].name;e=emoticons[smiley];}
return e?(b+callback(e[0],e[1],smiley,text)):a;});};emotify.emoticons=function(){var args=Array.prototype.slice.call(arguments),base_url=typeof args[0]==='string'?args.shift():'',replace_all=typeof args[0]==='boolean'?args.shift():false,smilies=args[0],e,arr=[],alts,i,regexp_str;if(smilies){if(replace_all){emoticons={};lookup=[];}
for(e in smilies){emoticons[e]=smilies[e];emoticons[e][0]=base_url+emoticons[e][0];}
for(e in emoticons){if(emoticons[e].length>2){alts=emoticons[e].slice(2).concat(e);i=alts.length
while(i--){alts[i]=alts[i].replace(/(\W)/g,'\\$1');}
regexp_str=alts.join('|');lookup.push({name:e,regexp:new RegExp('^'+regexp_str+'$')});}else{regexp_str=e.replace(/(\W)/g,'\\$1');}
arr.push(regexp_str);}
EMOTICON_RE=new RegExp('(^|\\s)('+arr.join('|')+')(?=(?:$|\\s))','g');}
return emoticons;};return emotify;})();(function(factory){"use strict";if(typeof define==='function'&&define.amd){define(['jquery'],function($){return factory($,window,document);});}
...
if(type=='alert'){console.log(msg);}
else if(type=='throw'){console.log(msg);}
...
if(navigator.vendor==='Apple Computer, Inc.'||(navigator.appCodeName||'')==='Mozilla'){History.Adapter.bind(window,'hashchange',function(){History.Adapter.trigger(window,'popstate');});if(History.getHash()){History.Adapter.onDomLoad(function(){History.Adapter.trigger(window,'hashchange');});}}}};if(!History.options||!History.options.delayInit){History.init();}})(window);(function defineMustache(global,factory){if(typeof exports==='object'&&exports&&typeof exports.nodeName!=='string'){factory(exports);}else if(typeof define==='function'&&define.amd){define(['exports'],factory);}else{global.Mustache={};factory(global.Mustache);}}(this,function mustacheFactory(mustache){var objectToString=Object.prototype.toString;var isArray=Array.isArray||function isArrayPolyfill(object){return objectToString.call(object)==='[object Array]';};function isFunction(object){return typeof object==='function';}
...
return defaultWriter.render(template,view,partials);};mustache.to_html=function to_html(template,view,partials,send){var result=mustache.render(template,view,partials);if(isFunction(send)){send(result);}else{return result;}};mustache.escape=escapeHtml;mustache.Scanner=Scanner;mustache.Context=Context;mustache.Writer=Writer;}));(function(factory){if(typeof define==='function'&&define.amd){define(['jquery'],factory);}else if(typeof module==='object'&&module.exports){module.exports=function(root,jQuery){if(jQuery===undefined){if(typeof window!=='undefined'){jQuery=require('jquery');}
...
return Select2;});return{define:S2.define,require:S2.require};}());var select2=S2.require('jquery.select2');jQuery.fn.select2.amd=S2;return select2;}));var token=$('input[name="__RequestVerificationToken"]').val();$.ajaxPrefilter(function(options,originalOptions){if(options.type.toUpperCase()=="POST"){options.data=$.param($.extend(originalOptions.data,{__RequestVerificationToken:token}));}});$.ajaxSetup({cache:false});$(document).ajaxError(function(e,xhr){if(xhr.status==403){var response=JSON.parse(xhr.responseText);window.location=response.LogOnUrl;}});function openModal(url,loading,data,callback){$.ajax({type:"post",url:url,data:data,cache:false,success:function(data,textStatus,jqXHR){if(data==""){handleError(jqXHR,"ClientNetworkError","");}else{$.modal(data,{autoResize:true,autoPosition:true,onClose:function(dialog,result){dialog.container.fadeOut(200,function(){dialog.overlay.fadeOut(150,function(){$.modal.close();if($.isFunction(callback)){callback(result);}});});},onOpen:function(dialog){dialog.overlay.fadeIn(150,function(){dialog.container.fadeIn(200);dialog.data.fadeIn(200);});}});}},error:handleError});}
function openModalGet(url,data,callback){$.ajax({type:"GET",url:url,data:data,cache:false,success:function(data,textStatus,jqXHR){if(data==""){handleError(jqXHR,"ClientNetworkError","");}else{$.modal(data,{autoResize:true,autoPosition:true,onClose:function(dialog,result){dialog.container.fadeOut("fast");dialog.data.slideUp("fast",function(){dialog.overlay.fadeOut("fast",function(){$.modal.close();if($.isFunction(callback)){callback(result);}});});},onOpen:function(dialog){dialog.overlay.fadeIn("fast",function(){dialog.container.fadeIn("fast");dialog.data.slideDown("fast",function(){});});}});}},error:handleError});}
function confirm(header,message,callbackYes,callbackNo){$('<div class="modalform modal-dialog">    <div class="modal-content">        <div class="modal-header">            <button class="close simplemodal-close" aria-hidden="true" type="button">×</button>            <h4 class="modal-title header"></h4>        </div>        <div class="modal-body">                <span class="message" >  </span>        </div>        <div class="modal-footer">            <button style="width:70px" class="btn btn-info yes " type="button">'+Resources.General.Yes+'</button>            <button style="width:70px" class="btn btn-info no" type="button">'+Resources.General.No+'</button>        </div>    </div></div>').modal({onShow:function(dialog){var modal=this;$('.header',dialog.data[0]).append(header);$('.message',dialog.data[0]).append(message);$('.yes',dialog.data[0]).click(function(){if($.isFunction(callbackYes)){callbackYes.apply();}
modal.close();});$('.no',dialog.data[0]).click(function(){if($.isFunction(callbackNo)){callbackNo.apply();}
modal.close();});},onClose:function(dialog,result){dialog.container.fadeOut("slow");dialog.data.slideUp("slow",function(){dialog.overlay.fadeOut("slow",function(){$.modal.close();});});},onOpen:function(dialog){dialog.overlay.fadeIn("slow",function(){dialog.container.fadeIn("slow");dialog.data.slideDown("slow",function(){});});}});}
function notify(header,message,callback){$('<div class="modalform modal-dialog">    <div class="modal-content">        <div class="modal-header">            <button class="close simplemodal-close" aria-hidden="true" type="button">×</button>            <h4 class="modal-title header"></h4>        </div>        <div class="modal-body">                <span class="message" >  </span>        </div>        <div class="modal-footer">            <button style="width:70px" class="btn btn-info ok" type="button">'+Resources.General.OK+'</button>                </div>    </div></div>').modal({onShow:function(dialog){var modal=this;$('.header',dialog.data[0]).append(header);$('.message',dialog.data[0]).append(message);$('.ok',dialog.data[0]).click(function(){if($.isFunction(callback)){callback.apply();}
modal.close();});},onClose:function(dialog,result){dialog.container.fadeOut("slow");dialog.data.slideUp("slow",function(){dialog.overlay.fadeOut("slow",function(){$.modal.close();});});},onOpen:function(dialog){dialog.overlay.fadeIn("slow",function(){dialog.container.fadeIn("slow");dialog.data.slideDown("slow",function(){});});}});}
function getJson(url,vars,callback){return $.ajax({url:url,cache:false,async:true,type:"post",dataType:'json',data:vars,success:function(response,textStatus,jqXHR){if(callback){callback(response);}},error:function(jqXHR,textStatus,errorThrown){}});}
function postJson(url,vars,callback,errorcallback){return $.ajax({url:url,cache:false,async:true,type:"POST",dataType:'json',data:vars,success:function(response,textStatus,jqXHR){if(callback){callback(response);}},error:function(jqXHR,textStatus,errorThrown){if(errorcallback){errorcallback(jqXHR,textStatus,errorThrown);}}});}
function postHtml(url,vars,callback){return $.ajax({url:url,cache:false,async:true,type:"post",dataType:'html',data:vars,success:function(response,textStatus,jqXHR){if(callback){callback(response);}},error:function(jqXHR,textStatus,errorThrown){}});}
function getJsonSync(url,vars,callback){return $.ajax({url:url,cache:false,async:false,type:"post",dataType:'json',data:vars,success:function(response,textStatus,jqXHR){if(callback){callback(response);}},error:function(jqXHR,textStatus,errorThrown){}});}
function postData(url,vars){return $.ajax({url:url,cache:false,async:false,type:"post",dataType:'html',data:vars});}
function getData(url,vars,callback){return $.ajax({url:url,cache:false,async:true,type:"get",dataType:'json',data:vars,success:function(response,textStatus,jqXHR){if(callback){callback(response);}},error:function(jqXHR,textStatus,errorThrown){}});}
function getPartial(div,url,showLoading,callback){if(showLoading){$(div).block({message:Resources.Layout.ModalLoadingMessage});}
$.ajax({url:url,cache:false,async:true,type:"GET",success:function(response,textStatus,jqXHR){$(div).html(response);if(showLoading){$(div).unblock();}
if(callback){callback(response);}},error:function(jqXHR,textStatus,errorThrown){if(showLoading){$(div).unblock();}}});}
function handleError(jqXHR,textStatus,errorThrown){if(jqXHR.status!=403){$.unblockUI();$.modal('<div class="modalform modal-dialog">    <div class="modal-content">        <div class="modal-header">            <button class="close simplemodal-close" aria-hidden="true" type="button">×</button>            <h4 class="modal-title">'+Resources.Layout.ModalInvalidRequestTitle+'</h4>        </div>        <div class="modal-body">            <p>'+Resources.Layout.modalInvalidRequestMessage1+'</p>            <p>'+Resources.Layout.modalInvalidRequestMessage2+'</p>        </div>         <div class="modal-footer">            <button style="width:70px" class="btn btn-info simplemodal-close" type="button">'+Resources.General.OK+'</button>        </div>    </div></div>');}};(function(factory){if(typeof define==='function'&&define.amd){define(['jquery'],factory);}else{factory(jQuery);}}
(function($){var d=[],doc=$(document),ua=navigator.userAgent.toLowerCase(),wndw=$(window),w=[];var browser={ieQuirks:null,msie:/msie/.test(ua)&&!/opera/.test(ua),opera:/opera/.test(ua)};browser.ie6=browser.msie&&/msie 6./.test(ua)&&typeof window['XMLHttpRequest']!=='object';browser.ie7=browser.msie&&/msie 7.0/.test(ua);$.modal=function(data,options){return $.modal.impl.init(data,options);};$.modal.close=function(data){$.modal.impl.close(data);};$.modal.focus=function(pos){$.modal.impl.focus(pos);};$.modal.setContainerDimensions=function(){$.modal.impl.setContainerDimensions();};$.modal.setPosition=function(){$.modal.impl.setPosition();};$.modal.update=function(height,width){$.modal.impl.update(height,width);};$.fn.modal=function(options){return $.modal.impl.init(this,options);};$.modal.defaults={appendTo:'body',focus:true,opacity:50,overlayId:'simplemodal-overlay',overlayCss:{},containerId:'simplemodal-container',containerCss:{},dataId:'simplemodal-data',dataCss:{},minHeight:null,minWidth:null,maxHeight:null,maxWidth:null,autoResize:true,autoPosition:true,zIndex:1000,close:true,closeHTML:'<a class="modalCloseImg" title="Close"></a>',closeClass:'simplemodal-close',escClose:true,overlayClose:false,fixed:false,position:null,persist:false,modal:true,onOpen:null,onShow:null,onClose:null};$.modal.impl={d:{},init:function(data,options){var s=this;if(s.d.data){return false;}
...
s.o.focus&&s.focus();s.bindEvents();},close:function(data){var s=this;if(!s.d.data){return false;}
s.unbindEvents();if($.isFunction(s.o.onClose)&&!s.occb){s.occb=true;s.o.onClose.apply(s,[s.d,data]);}else{if(s.d.placeholder){var ph=$('#simplemodal-placeholder');if(s.o.persist){ph.replaceWith(s.d.data.removeClass('simplemodal-data').css('display',s.display));}else{s.d.data.hide().remove();ph.replaceWith(s.d.orig);}}else{s.d.data.hide().remove();}
s.d.container.hide().remove();s.d.overlay.hide();s.d.iframe&&s.d.iframe.hide().remove();s.d.overlay.remove();s.d={};return data;}}};}));jQuery.extend(jQuery.fn.dataTableExt.oSort,{"title-numeric-pre":function(a){var x=a.match(/title="*(-?[0-9\.]+)/)[1];return parseFloat(x);},"title-numeric-asc":function(a,b){return((a<b)?-1:((a>b)?1:0));},"title-numeric-desc":function(a,b){return((a<b)?1:((a>b)?-1:0));}});!function(factory){"function"==typeof define&&define.amd?define(["jquery"],factory):factory("object"==typeof exports?require("jquery"):jQuery);}(function($){var caretTimeoutId,ua=navigator.userAgent,iPhone=/iphone/i.test(ua),chrome=/chrome/i.test(ua),android=/android/i.test(ua);$.mask={definitions:{"9":"[0-9]",a:"[A-Za-z]","*":"[A-Za-z0-9]"},autoclear:!0,dataName:"rawMaskFn",placeholder:"_"},$.fn.extend({caret:function(begin,end){var range;if(0!==this.length&&!this.is(":hidden"))return"number"==typeof begin?(end="number"==typeof end?end:begin,this.each(function(){this.setSelectionRange?this.setSelectionRange(begin,end):this.createTextRange&&(range=this.createTextRange(),range.collapse(!0),range.moveEnd("character",end),range.moveStart("character",begin),range.select());})):(this[0].setSelectionRange?(begin=this[0].selectionStart,end=this[0].selectionEnd):document.selection&&document.selection.createRange&&(range=document.selection.createRange(),begin=0-range.duplicate().moveStart("character",-1e5),end=begin+range.text.length),{begin:begin,end:end});},unmask:function(){return this.trigger("unmask");},mask:function(mask,settings){var input,defs,tests,partialPosition,firstNonMaskPos,lastRequiredNonMaskPos,len,oldVal;if(!mask&&this.length>0){input=$(this[0]);var fn=input.data($.mask.dataName);return fn?fn():void 0;}
...
var input=$(this),buffer=$.map(mask.split(""),function(c,i){return"?"!=c?defs[c]?getPlaceholder(i):c:void 0;}),defaultBuffer=buffer.join(""),focusText=input.val();input.data($.mask.dataName,function(){return $.map(buffer,function(c,i){return tests[i]&&c!=getPlaceholder(i)?c:null;}).join("");}),input.one("unmask",function(){input.off(".mask").removeData($.mask.dataName);}).on("focus.mask",function(){if(!input.prop("readonly")){clearTimeout(caretTimeoutId);var pos;focusText=input.val(),pos=checkVal(),caretTimeoutId=setTimeout(function(){input.get(0)===document.activeElement&&(writeBuffer(),pos==mask.replace("?","").length?input.caret(0,pos):input.caret(pos));},10);}}).on("blur.mask",blurEvent).on("keydown.mask",keydownEvent).on("keypress.mask",keypressEvent).on("input.mask paste.mask",function(){input.prop("readonly")||setTimeout(function(){var pos=checkVal(!0);input.caret(pos),tryFireCompleted();},0);}),chrome&&android&&input.off("input.mask").on("input.mask",androidInputEvent),checkVal();});}});});var notificationTemplate=$("#notificationTemplate").html();function initializeWebsocket(){notificationHub.client.SendNotification=function(notification){sendNotification(notification.Header,notification.Notification,notification.Type);};notificationHub.client.SendDataNotification=function(notification){$(document).trigger(notification.Event,notification);$(document).trigger(notification.Event+"Global",notification);};$.connection.hub.start({transport:['webSockets']});}
function sendNotification(header,message,type){var html=Mustache.render(notificationTemplate,{header:header,message:message,type:notificationTypeToText(type),icon:notificationTypeToIcon(type)});$.jGrowl(html,{position:"bottom-right"});}
function notificationTypeToIcon(type){if(type===1){return"fa-exclamation-triangle";}else if(type===2){return"fa-times";}else if(type===3){return"fa-check-circle-o";}
return"fa-info-circle";}
function notificationTypeToText(type){if(type===1){return"warning";}else if(type===2){return"error";}else if(type===3){return"success";}
return"info";}
function cancelOrder(tradeId,tradePairId){var data={tradeId:tradeId,tradePairId:tradePairId};$.blockUI({message:Resources.Layout.BlockCancelOrderMessage});getJson('/Exchange/CancelTrade',data,orderCanceled);}
function cancelAllOrders(){confirm(Resources.Layout.ConfirmCancelAllOrdersTitle,Resources.Layout.confirmCancelAllOrdersMessage,function(){$.blockUI({message:Resources.Layout.BlockCancelAllOrdersMessage});var data={};getJson('/Exchange/CancelAllTrades',data,allordersCanceled);});}
function cancelTradePairOrders(tradePairId){confirm(Resources.Layout.ConfirmCancelTradePairTitle,Resources.Layout.ConfirmCancelTradePairTitleMessage,function(){$.blockUI({message:Resources.Layout.BlockCancelTradePairMessage});var data={tradePairId:tradePairId};getJson('/Exchange/CancelTradePairTrades',data,allordersCanceled);});}
function orderCanceled(response){$.unblockUI();}
function allordersCanceled(response){$.unblockUI();}
function htmlEncode(val){return $('<div/>').text(val).html();}
function scrollToAnchor(aid){var aTag=$("a[name='"+aid+"']");if(aTag&&aTag.offset()){$('html, body').animate({scrollTop:aTag.offset().top},'fast');}}
$("#theme-switch").on("click",function(){var url=$(this).data("action");postJson(url,{},function(data){if(data.Success){changeTheme(data.Message);}});});function changeTheme(theme){if(theme=='Dark'){$("#theme-switch").attr('title',Resources.Layout.ThemeLight);$("#theme-switch i").removeClass('icon-moon-night').addClass('icon-lightbulb-idea');$('link[title="siteTheme"]').attr({href:"/Content/theme.Dark.css"});}
else if(theme=='Light'){$("#theme-switch").attr('title',Resources.Layout.ThemeDark);$("#theme-switch i").removeClass('icon-lightbulb-idea').addClass('icon-moon-night');$('link[title="siteTheme"]').attr({href:"/Content/theme.Light.css"});}}
...
$(window).bind('resize load',syncHeights);syncHeights();return this;};})(jQuery);if(!String.linkify){String.prototype.linkify=function(){var urlPattern=/\b(?:https?|ftp):\/\/[a-z0-9-+&@#\/%?=~_|!:,.;]*[a-z0-9-+&@#\/%=~_|]/gim;var pseudoUrlPattern=/(^|[^\/])(www\.[\S]+(\b|$))/gim;var emailAddressPattern=/[\w.]+@[a-zA-Z_-]+?(?:\.[a-zA-Z]{2,6})+/gim;if(this.match(/bitcointalk.org/g)){return'<a rel="noopener noreferrer" target="_blank" href="https://www.youtube.com/watch?v=dQw4w9WgXcQ">'+Resources.Layout.LinkifyForbiddenLink+'</a>';}
...
return s;};function getStarRating(ratingValue){var rating="";if(ratingValue===""||ratingValue==-1){for(i=0;i<5;i++){rating+='<i title="'+Resources.Layout.RatingUnrated+'" class="fa fa-star-o"></i>';}
return rating;}
if(ratingValue==0){for(i=0;i<5;i++){rating+='<i title="0/5" class="fa fa-star-o text-danger"></i>';}
return rating;}
for(i=0;i<5;i++){var title=Resources.Layout.RatingRated+' '+ratingValue+'/5';if(i<ratingValue){if(ratingValue%1!=0&&ratingValue-0.5==i){if(ratingValue>=5){rating+='<i title="'+title+'" class="fa fa-star-half-o text-warning"></i>';}else if(ratingValue>2){rating+='<i title="'+title+'" class="fa fa-star-half-o text-success"></i>';}else{rating+='<i title="'+title+'" class="fa fa-star-half-o text-danger"></i>';}}else{if(ratingValue>=5){rating+='<i title="'+title+'" class="fa fa-star text-warning"></i>';}else if(ratingValue>2){rating+='<i title="'+title+'" class="fa fa-star text-success"></i>';}else{rating+='<i title="'+title+'" class="fa fa-star text-danger"></i>';}}}else{rating+='<i title="'+title+'" class="fa fa-star-o"></i>';}}
return rating;}
function showMessage(data){if(data&&!data.Cancel){var message=data.Message||(Resources.Layout.ErrorContactSupportText+' <a href="/UserSupport">'+Resources.Layout.ErrorContactSupportLink+'.</a>');var alert=$("#message-alert");alert.show();alert.addClass(data.Success?"alert-success":"alert-danger");alert.find("p").html(message);alert.fadeTo(5000,500).slideUp(500,function(){alert.find("p").html("");alert.removeClass("alert-danger alert-success").hide();});}}
setInterval(function(){$(".servertime-label").text(moment.utc().format("D/MM/YYYY h:mm:ss A"))},1000);function toLocalTime(time){return moment.utc(time).local().format("D/MM/YYYY h:mm:ss A")}
function toLocalDate(date){return moment.utc(date).local().format("D/MM/YYYY")}
$(function(){$("body").tooltip({selector:'[data-toggle=tooltip]'});$("body").popover({selector:'[data-toggle=popover]'});$('#notification-menu').on('mouseenter',function(){getData('/UserNotification/GetNotificationMenu',{},function(data){var notifications=$("#notification-menu-notifications");notifications.empty();if(data&&data.length===0){notifications.append("<div style='text-align:center;font-size:11px'><i>"+Resources.Layout.MenuNoNotification+"</i></div>");return;}
for(var i in data){notifications.append("<div style='font-size:11px'>"+moment.utc(data[i].Timestamp).local().fromNow()+": "+data[i].Title+" - "+data[i].Notification+"</div>")}});});$('#notification-menu-clear').on('click',function(){getJson('/UserNotification/Clear',{},function(data){$('.notification-menu-count').html('');$("#notification-menu-notifications").empty();});});if(authenticated=='True'){updateMessageCount();}});function updateMessageCount(){getData('/UserNotification/GetNotificationCount',{},function(data){$('.notification-menu-message-count').html(data.MessageCount==0?"":data.MessageCount>1000?"999+":data.MessageCount);$('.notification-menu-count').html(data.NotificationCount==0?"":data.NotificationCount>1000?"999+":data.NotificationCount);});}
function setSelectedNavOption(title){if(title){$('#nav-header').removeClass('active');$('.nav-'+title.toLowerCase()).addClass('active');}}
function printObj(object){var simpleObject={};for(var prop in object){if(!object.hasOwnProperty(prop)){continue;}
if(typeof(object[prop])=='object'){continue;}
if(typeof(object[prop])=='function'){continue;}
simpleObject[prop]=object[prop];}
return JSON.stringify(simpleObject);};var datatableExportLayout="<'row'<'col-sm-6'><'col-sm-6'f>><'row'<'col-sm-12'tr>><'datatable-length-row'l> <'datatable-export-row'B><'datatable-page-row'p><'clearfix'><'datatable-info-row'i>";function datatableExportButtons(filename){return[{extend:'csvHtml5',text:'<span data-toggle="tooltip" data-container="body" title="'+Resources.Layout.ExportCsvButton+'" class="fa fa-file-text-o"></span>',filename:filename.replace(" ","_")},{extend:'excelHtml5',text:'<span data-toggle="tooltip" data-container="body" title="'+Resources.Layout.ExportExcelButton+'" class="fa fa-file-excel-o"></span>',filename:filename.replace(" ","_")},{extend:'pdfHtml5',text:'<span data-toggle="tooltip" data-container="body" title="'+Resources.Layout.ExportPdfButton+'" class="fa fa-file-pdf-o"></span>',filename:filename.replace(" ","_"),title:'Cryptopia '+filename},{extend:'copy',text:'<span data-toggle="tooltip" data-container="body" title="'+Resources.Layout.ExportClipboardButton+'" class="fa fa-files-o"></span>',},{extend:'print',text:'<span data-toggle="tooltip" data-container="body" title="'+Resources.Layout.ExportPrintButton+'" class="fa fa-print"></span>',title:'Cryptopia '+filename}]};function triggerWindowResize(){if(typeof(Event)==='function'){window.dispatchEvent(new Event('resize'));}else{var evt=window.document.createEvent('UIEvents');evt.initUIEvent('resize',true,false,window,0);window.dispatchEvent(evt);}}
var store=new Storage();function Storage(authenticated){this.get=function(name){return JSON.parse(window.localStorage.getItem(name));};this.set=function(name,value){window.localStorage.setItem(name,JSON.stringify(value));};this.clear=function(){window.localStorage.clear();};}
function changeHighlight(change){return change>0?"text-success":change<0?"text-danger":"";}
function highlightRow(element,highlight){if(element.hasClass("greenhighlight")||element.hasClass("redhighlight")||element.hasClass("bluehighlight")){element.removeClass("greenhighlight redhighlight bluehighlight").addClass(highlight+"highlight2");}else{element.removeClass("greenhighlight2 redhighlight2 bluehighlight2").addClass(highlight+"highlight")}}
function highlightRowText(element,highlight){if(element.hasClass("greenhighlighttext")||element.hasClass("redhighlighttext")||element.hasClass("bluehighlighttext")){element.removeClass("greenhighlighttext redhighlighttext bluehighlighttext").addClass(highlight+"highlighttext2");}else{element.removeClass("greenhighlighttext2 redhighlighttext2 bluehighlighttext2").addClass(highlight+"highlighttext")}}
function highlightItem(element,highlight){if(element.hasClass("info")){highlightRowText(element,highlight);}
else{highlightRow(element,highlight);}}
function highlightRemove(selector){$(selector).removeClass("greenhighlighttext2 redhighlighttext2 bluehighlighttext2 greenhighlighttext redhighlighttext bluehighlighttext greenhighlight redhighlight bluehighlight greenhighlight2 redhighlight2 bluehighlight2");}