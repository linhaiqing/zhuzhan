...
$('#count').keyup(function(){
    ...
    if(strPrice.indexOf('.') > 0 && strCount.indexOf('.')<0){ //单价浮点型 数量整型
        ...
        mum    = ((price*Math.pow(10,np))*(count*Math.pow(10,np)))/Math.pow(10,np*2);

    }

    if(strPrice.indexOf('.') < 0 && strCount.indexOf('.') > 0){ //单价整型 数量浮点型
        var nc = count.toString().split(".")[1].length;
        mum    = ((price*Math.pow(10,nc))*(count*Math.pow(10,nc)))/Math.pow(10,nc*2);
    }

    if(strPrice.indexOf('.') > 0 && strCount.indexOf('.') > 0){ // 单价数量 皆为浮点型
        var np    = price.toString().split(".")[1].length;
        var nc    = count.toString().split(".")[1].length;
        mum    = ((price*Math.pow(10,np))*(count*Math.pow(10,nc)))/Math.pow(10,nc+np);
    }
...
function form_up(){
   ...
   if(coin == '' || coin == null){
      $('.layui-select-title .layui-unselect').focus();
